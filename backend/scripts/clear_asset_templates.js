...
async function main() {
  ...
  if (!superAdminEmail) {
    console.error(
      "Usage: node scripts/clear_asset_templates.js <SuperAdminEmail>",
    );
    process.exit(1);
  }
  ...
  console.log(
    ...
    templates
      .slice(0, 10)
      .map((t) => `${t.name} (bu:${t.businessUnitId})`)
      .join(", "),
  );
  ...
  if (process.env.FORCE !== "1") {
    ...
    const answer = await new Promise((resolve) =>
      rl.question("Type DELETE to confirm: ", (a) => {
        rl.close();
        resolve(a);
      }),
    );
  ...
  try {
    ...
    const results = await prisma.$transaction([
      ...
      prisma.purchaseOrderItem.updateMany({
        where: { assetTemplateId: { in: ids } },
        data: { assetTemplateId: null },
      }),
